...
class Crazy:
    ...
    def moveWaypoint(self, dir, amt):
        if dir=='E':
            self.eWay=self.eWay+amt
        elif dir=='W':
            self.wWay=self.wWay+amt
        elif dir=='N':
            self.nWay=self.nWay+amt
        elif dir=='S':
            self.sWay=self.sWay+amt

    def rotateWaypoint(self, dir, deg):
        d=deg/90
        d=int(d%4)
        for i in range(d):
            if dir=='R':
    ...
    def computeDistance(self):
        manDist=abs(self.eMoved-self.wMoved)+abs(self.nMoved-self.sMoved)
        return manDist
    ...
    def move(self, amt):
        ...
        self.wMoved=self.wMoved+self.wWay*amt
        self.nMoved=self.nMoved+self.nWay*amt
        self.sMoved=self.sMoved+self.sWay*amt

crazyMovements=Crazy()
...
for line in directions:
    ...
    if inst in news:
        crazyMovements.moveWaypoint(inst, num)
    elif inst=='L' or inst=='R':
        crazyMovements.rotateWaypoint(inst, num)
    elif inst=='F':
        crazyMovements.move(num)
    
    crazyMovements.printMovement()

print (crazyMovements.computeDistance())
