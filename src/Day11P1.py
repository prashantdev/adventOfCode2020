...
def getNumFullSurroundingSeats(lay, r, c):
    # r-1 check:
    numFull=0
    if r>0:#if zero, r-1 has no occupants
        # c-1 check
        if c>0 and not isSeatEmpty(lay[r-1][c-1]):
            numFull=numFull+1
        # c check
...
def setSeat(oldLay, newLay, rowIndex, colIndex, value):
    if (oldLay[rowIndex][colIndex]=='.'):
        newLay[rowIndex][colIndex]='.'
    else:
        newLay[rowIndex][colIndex]=value
...
def printLayout(lay):
    ...
    for row in lay:
        joinedRow="".join(row)
        #print(joinedRow)
        countOccupied=countOccupied+joinedRow.count('#')
    print("Occupied="+str(countOccupied))

def createLayout():
    layRow = ['-'] * numCols
    lay=[]
    for i in range(numRows):
        lay.append(list(layRow))
    return lay

def seatingIteration(lay):
    newLay=createLayout()
...
oldLayout=layout
while change:
    newLayout=seatingIteration(oldLayout)
